...
usage() {
  cat <<'EOF'
Usage: ./environment_prepare_usb.sh [--verbose|-v]
...
mkdir -p "$LOG_DIR"
BUILD_LOG="${LOG_DIR}/prepare_usb_$(date +%Y%m%d_%H%M%S).log"
...
# USB flow still referenced the PCIe module in original script; keep best-effort behavior
...
# 3) Build USB kernel driver
section "Kernel Driver (usb)"
info "Building kernel driver (usb)"
require_file "./1_kdrv_src/usb/Makefile"

pushd "./1_kdrv_src/usb" >/dev/null
run_make make clean
run_make make all
require_file "./memx_cascade_usb.ko"
run cp "./memx_cascade_usb.ko" "../memx_cascade_plus.ko"
run_make make clean
popd >/dev/null

# 4) Prepare include dir and remove PCIe flash tools
section "System Paths & Tools"
info "Ensuring /usr/include/memx exists"
sudo_run mkdir -p "/usr/include/memx"

info "Removing PCIe flash tools (if present)"
run rm -rf "./3_flashupdate/pcieupdateflash"
run rm -rf "./3_flashupdate/read_fwver"
run rm -rf "./3_flashupdate/check_version"

# 5) Install user driver lib + headers + USB flash tool (arch-specific)
...
case "$arch" in
  x86_64)
    ...
    require_dir  "./3_flashupdate/x86_64/memx_usb_update_flash_tool"

    sudo_run cp "./2_udrv_lib/x86_64/libmemx.so" "/usr/lib/"
    sudo_run cp "./2_udrv_lib/x86_64/memx.h" "/usr/include/memx/"
    run cp -rf "./3_flashupdate/x86_64/memx_usb_update_flash_tool" "./3_flashupdate/memx_usb_update_flash_tool"
    ;;
  ...
  aarch64)
    ...
    require_dir  "./3_flashupdate/aarch64/memx_usb_update_flash_tool"

    sudo_run cp "./2_udrv_lib/aarch64/libmemx.so" "/usr/lib/"
    sudo_run cp "./2_udrv_lib/aarch64/memx.h" "/usr/include/memx/"
    run cp -rf "./3_flashupdate/aarch64/memx_usb_update_flash_tool" "./3_flashupdate/memx_usb_update_flash_tool"
    ;;
